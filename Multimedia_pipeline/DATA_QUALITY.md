...
- **Missing event GPS — [#16](https://github.com/svenbuerki/Genetic-Rescue-DB/issues/16):** events **405** (EO32) and **451** (EO30-2) have NULL lat/long — left **blank on the forms** (boards carry no GPS). Coordinates left blank (no estimates); each event carries an `eventRemarks` flag; crew to supply from the field GPS unit. (Older: event 13, 2025.)
